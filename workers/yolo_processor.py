...
# ==========================================
# 3. MAIN PROCESSOR (Bulletproof Core)
# ==========================================
class YoloProcessor(QThread):
    ...
    def run(self):
        ...
        # --- 3-TIER BULLETPROOF HARDWARE TESTER ---
        device_target = 'cpu'
        use_half = False

        if torch.cuda.is_available():
            self.log_message.emit(f"🔍 Checking Hardware: {torch.cuda.get_device_name(0)}")
            dummy_img = np.zeros((160, 160, 3), dtype=np.uint8) # Fake image to test the GPU

            try:
                # TIER 1: Try Max Speed FP16
        ...
        for idx, video_path in enumerate(self.video_files):
            ...
            try:
                ...
                while self.is_running:
                    ...
                    # Uses whatever hardware passed the stress test above
