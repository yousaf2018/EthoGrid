...
class VideoSaver(QThread):
    ...
    def __init__(self, source_video_path, output_video_path, detections, 
                 ...
                 timeline_segments, draw_grid=False, draw_overlays=True, 
                 draw_polygons=True, parent=None): # Added draw_polygons
        ...
        self.draw_polygons = draw_polygons # Store the flag
    ...
    def _get_clipped_mask(self, polygon_str, tank_number):
        try:
            ...
            # Calculate grid cell from tank number
        ...
        if frame_idx in self.detections:
            for det in self.detections[frame_idx]:
                if det.get('tank_number') is not None and (not self.selected_cells or str(det['tank_number']) in self.selected_cells):
                    ...
                    # ### THE FIX: Check self.draw_polygons ###
                    if self.draw_polygons and 'polygon' in det and det['polygon']:
                    ...
                    # Enhanced label with Track ID if available
                    label = f"T{int(det['tank_number'])}"
                    if 'track_id' in det:
                         label += f" ID:{int(det['track_id'])}"
                    
                    font_face, f_scale, f_thick = cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2
