...
class VideoSaver(QThread):
    ...
    def __init__(self, source_video_path, output_video_path, detections, 
        ...
        # ### CORRECTED CANVAS SIZE CALCULATION ###
        ...
        if self.draw_overlays:
            ...
            # The final width is the LARGER of the original video width or the legend's required width.
            # The legend is drawn to the right, so we need space for it.
            # The timeline is drawn below, so its width should match the combined video+legend width.
    ...
    def _draw_timeline_on_frame(self, frame, frame_idx, total_frames, original_video_height):
        ...
        # Draw the timeline background across the full width of the new canvas
        cv2.rectangle(frame, (0, original_video_height), (new_w, new_h), (10, 10, 10), -1)
        
        # Calculate drawing area for the bars
    ...
    def process_frame(self, original_frame, frame_idx, total_frames):
        ...
        if self.draw_overlays:
            ...
            processed_frame = np.full((new_h, new_w, 3), 43, dtype=np.uint8) # Dark grey background
                ...
                if det.get('tank_number') is not None and (not self.selected_cells or str(det['tank_number']) in self.selected_cells):
                    ...
                    if 'polygon' in det and det['polygon']:
                        try:
                            ...
                            x1, y1, x2, y2 = map(int, (det["x1"], det["y1"], det["x2"], det["y2"])); cv2.rectangle(processed_frame, (x1, y1), (x2, y2), color_bgr, 2)
                    else:
                        x1, y1, x2, y2 = map(int, (det["x1"], det["y1"], det["x2"], det["y2"])); cv2.rectangle(processed_frame, (x1, y1), (x2, y2), color_bgr, 2)
                    if det.get('cx') is not None and det.get('cy') is not None: cv2.circle(processed_frame, (int(det['cx']), int(det['cy'])), 8, (0, 0, 255), -1)
                    label = f"{det['tank_number']}"; font_face, f_scale, f_thick = cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2; x1, y1 = int(det["x1"]), int(det["y1"])
                    (tw, th), _ = cv2.getTextSize(label, font_face, f_scale, f_thick)
                    cv2.rectangle(processed_frame, (x1, y1 - th - 12), (x1 + tw, y1), color_bgr, -1); cv2.putText(processed_frame, label, (x1, y1 - 7), font_face, f_scale, (0,0,0), f_thick, cv2.LINE_AA)
