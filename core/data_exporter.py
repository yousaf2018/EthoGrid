    ...
    try:
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                tank_num = r * cols + c + 1
                cv2.putText(untransformed_layer, f"Tank {tank_num}", (x1 + 15, y1 + 40), 
        ...
        for det in all_dets:
            ...
            if tank_num is not None and cx is not None and cy is not None:
                ...
                tank_points[int(tank_num)].append({
    ...
    try:
        ...
        for frame_idx in range(all_frames[0], all_frames[-1] + 1):
            ...
            for tank_idx in range(total_tanks):
                ...
                row_dict[f'x{tank_idx}'] = cx_str; row_dict[f'y{tank_idx}'] = cy_str
            output_rows.append(row_dict)
        output_df = pd.DataFrame(output_rows); output_df.to_csv(output_path, index=False)
        return None
    except Exception as e:
