...
class BatchProcessDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.add_videos_btn = QtWidgets.QPushButton("Add Videos..."); self.browse_settings_btn = QtWidgets.QPushButton("Browse..."); self.browse_output_btn = QtWidgets.QPushButton("Browse...")
        ...
        self.save_csv_checkbox = QtWidgets.QCheckBox("Save Enriched CSV"); self.save_csv_checkbox.setChecked(True)
        ...
        output_options_layout.addWidget(self.save_video_checkbox)
        output_options_layout.addWidget(self.show_overlays_checkbox)
        output_options_layout.addWidget(self.save_csv_checkbox)
        output_options_layout.addWidget(self.save_centroid_csv_checkbox)
        form_layout.addWidget(output_options_group, 6, 0, 1, 2)
        ...
        file_progress_layout = QtWidgets.QHBoxLayout(); file_progress_layout.addWidget(QtWidgets.QLabel("Current File Progress:")); file_progress_layout.addWidget(self.file_progress_label); file_progress_layout.addStretch()
        ...
        self.add_videos_btn.clicked.connect(self.add_videos); self.browse_settings_btn.clicked.connect(self.browse_settings); self.browse_output_btn.clicked.connect(self.browse_output)
        self.start_btn.clicked.connect(self.start_processing); self.cancel_btn.clicked.connect(self.cancel_processing)
        self.cancel_btn.setEnabled(False)
        self.save_video_checkbox.stateChanged.connect(lambda state: self.show_overlays_checkbox.setEnabled(state > 0))
    ...
    def start_processing(self):
        ...
        if not self.save_video_checkbox.isChecked() and not self.save_csv_checkbox.isChecked() and not self.save_centroid_csv_checkbox.isChecked():
        ...
        self.batch_worker.overall_progress.connect(self.update_overall_progress); self.batch_worker.file_progress.connect(self.update_file_progress); self.batch_worker.log_message.connect(self.log_text_edit.append); self.batch_worker.finished.connect(self.on_processing_finished); self.batch_thread.started.connect(self.batch_worker.run)
        self.batch_thread.start()

    def cancel_processing(self):
        if self.batch_worker: self.batch_worker.stop()
        self.cancel_btn.setEnabled(False)
    ...
    def toggle_controls(self, enabled):
        self.start_btn.setEnabled(enabled); self.add_videos_btn.setEnabled(enabled); self.browse_settings_btn.setEnabled(enabled); self.browse_output_btn.setEnabled(enabled)
