...
class BatchProcessDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        # ### FIX 1: INCREASED RANGE ###
        self.time_gap_spinbox.setRange(0.1, 99999.0) 
        self.time_gap_spinbox.setValue(1.0)
        self.time_gap_spinbox.setSingleStep(0.1)
        ...
        form_layout.addWidget(output_options_group, 6, 0, 1, 2); layout.addLayout(form_layout)
        ...
        # ### FIX 2: ROBUST CHECKBOX LOGIC ###
        self.save_video_checkbox.stateChanged.connect(self.on_save_video_changed)
        self.save_trajectory_img_checkbox.stateChanged.connect(self.on_save_trajectory_changed)
        # Initial state check
        self.on_save_video_changed()
        self.on_save_trajectory_changed()


    # ### NEW METHODS FOR CHECKBOX LOGIC ###
    def on_save_video_changed(self):
        ...
        if not is_checked:
            self.show_overlays_checkbox.setChecked(False)

    def on_save_trajectory_changed(self):
        is_checked = self.save_trajectory_img_checkbox.isChecked()
        self.time_gap_spinbox.setEnabled(is_checked)
    ...
    def toggle_controls(self, enabled):
        self.start_btn.setEnabled(enabled); self.add_videos_btn.setEnabled(enabled); self.browse_settings_btn.setEnabled(enabled); self.browse_output_btn.setEnabled(enabled); self.cancel_btn.setEnabled(not enabled)
