...
class YoloInferenceDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.model_line_edit = QtWidgets.QLineEdit()
        self.model_line_edit.setPlaceholderText("Click 'Browse' to select a YOLO model file (.pt)")
        self.output_dir_line_edit = QtWidgets.QLineEdit()
        self.output_dir_line_edit.setPlaceholderText("Click 'Browse' to select an output folder")
        self.add_videos_btn = QtWidgets.QPushButton("Add Videos...")
        ...
        self.confidence_spinbox.setValue(0.4)
        self.save_video_checkbox = QtWidgets.QCheckBox("Save Annotated Video")
        self.save_video_checkbox.setChecked(True)
        self.save_csv_checkbox = QtWidgets.QCheckBox("Save Detections CSV")
        ...
        self.file_progress_label = QtWidgets.QLabel("Frame: 0 / 0")
        ...
        form_layout.addWidget(QtWidgets.QLabel("Video Files:"), 0, 0)
        form_layout.addWidget(self.video_list_widget, 1, 0, 1, 2)
        form_layout.addWidget(self.add_videos_btn, 1, 2)
        form_layout.addWidget(QtWidgets.QLabel("YOLO Model File (.pt):"), 2, 0)
        form_layout.addWidget(self.model_line_edit, 3, 0)
        form_layout.addWidget(self.browse_model_btn, 3, 1)
        form_layout.addWidget(QtWidgets.QLabel("Output Directory:"), 4, 0)
        form_layout.addWidget(self.output_dir_line_edit, 5, 0)
        form_layout.addWidget(self.browse_output_btn, 5, 1)
        form_layout.addWidget(QtWidgets.QLabel("Confidence Threshold:"), 6, 0)
        form_layout.addWidget(self.confidence_spinbox, 6, 1)
        output_options_group = QtWidgets.QGroupBox("Output Options")
        output_options_layout = QtWidgets.QHBoxLayout(output_options_group)
        ...
        file_progress_layout.addWidget(QtWidgets.QLabel("Current Video Progress:"))
    ...
    def add_videos(self):
        ...
        if files:
            ...
            self.video_list_widget.addItems([os.path.basename(f) for f in files])

    def browse_model(self):
        file, _ = QtWidgets.QFileDialog.getOpenFileName(self, "Select YOLO Model", "", "PyTorch Models (*.pt)")
        if file:
            self.model_line_edit.setText(file)
