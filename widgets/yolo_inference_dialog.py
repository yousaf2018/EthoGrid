...
class YoloInferenceDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.setMinimumSize(700, 550) # Increased height slightly for new options
        self.video_files = []
    ...
    def start_processing(self):
        ...
        # ### NEW VALIDATION ###
        if not self.save_video_checkbox.isChecked() and not self.save_csv_checkbox.isChecked():
            QtWidgets.QMessageBox.warning(self, "Input Error", "Please select at least one output option (Save Video or Save CSV)."); return

        self.toggle_controls(False); self.log_text_edit.clear()
        
        # ### PASS NEW OPTIONS TO WORKER ###
