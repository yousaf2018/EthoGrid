...
class FileListWidget(QtWidgets.QWidget):
    ...
    def get_full_paths(self):
        return [self.file_list.item(i).data(QtCore.Qt.UserRole) for i in range(self.file_list.count())]
...
class StatsDialog(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.group_widgets = {} # {group_name: FileListWidget}
        ...
        # --- LEFT PANE ---
        self.input_group = QtWidgets.QGroupBox("1. Input Data & Groups")
        self.input_layout = QtWidgets.QVBoxLayout(self.input_group)
        self.add_group_button = QtWidgets.QPushButton("Add New Group...")
        self.input_layout.addWidget(self.add_group_button)
        left_pane.addWidget(self.input_group)
        
        level_group = QtWidgets.QGroupBox("2. Select Analysis Level")
        level_layout = QtWidgets.QFormLayout(level_group)
        self.level_combo = QtWidgets.QComboBox()
        self.level_combo.addItems(["Compare Grand Averages", "Compare Sheet Averages", "Compare Tanks"])
        level_layout.addRow(self.level_combo)
        left_pane.addWidget(level_group)

        endpoints_group = QtWidgets.QGroupBox("3. Endpoints to Analyze")
        ...
        endpoints_layout.addWidget(self.endpoints_scroll)
        endpoints_layout.addWidget(self.select_all_btn)
        left_pane.addWidget(endpoints_group)

        params_group = QtWidgets.QGroupBox("4. Analysis Parameters")
        ...
        params_layout.addRow("Normality Test:", self.normality_test_combo)
        params_layout.addRow("Alpha (p-value):", self.alpha_spinbox)
        params_layout.addRow("If Normal:", self.parametric_label)
        params_layout.addRow("If Not Normal:", self.nonparametric_label)
        params_layout.addRow(self.force_parametric_cb)
        left_pane.addWidget(params_group)

        plot_group = QtWidgets.QGroupBox("5. Plot Customization")
        ...
        self.plot_width_spinbox = QtWidgets.QSpinBox(value=800, minimum=400, maximum=3000, singleStep=50, suffix=" px")
        self.plot_height_spinbox = QtWidgets.QSpinBox(value=600, minimum=400, maximum=3000, singleStep=50, suffix=" px")
        self.plot_dpi_spinbox = QtWidgets.QSpinBox(value=300, minimum=72, maximum=600, singleStep=10, suffix=" DPI")
        self.title_size_spinbox = QtWidgets.QSpinBox(value=16, minimum=8, maximum=30); self.axes_size_spinbox = QtWidgets.QSpinBox(value=12, minimum=6, maximum=24)
        self.tick_size_spinbox = QtWidgets.QSpinBox(value=10, minimum=6, maximum=20)
        ...
        plot_layout.addRow("Bar Height:", self.central_tendency_combo)
        plot_layout.addRow("Error Bar Type:", self.error_bar_combo)
        plot_layout.addRow("Color Palette:", self.palette_combo)
        plot_layout.addRow("Plot Width / Height (px):", self.create_hbox(self.plot_width_spinbox, self.plot_height_spinbox))
        plot_layout.addRow("Plot DPI:", self.plot_dpi_spinbox)
        plot_layout.addRow("Title Font Size / Weight:", self.create_hbox(self.title_size_spinbox, self.title_weight_combo))
        plot_layout.addRow("Axes Label Font Size / Weight:", self.create_hbox(self.axes_size_spinbox, self.axes_weight_combo))
        plot_layout.addRow("Axes Tick Font Size:", self.tick_size_spinbox)
        left_pane.addWidget(plot_group)
        ...
        top_splitter.addWidget(left_pane_scroll); top_splitter.addWidget(right_pane_container); top_splitter.setSizes([400, 600])
        ...
        self.start_btn = QtWidgets.QPushButton("Run Analysis")
        ...
        self.select_all_btn.clicked.connect(self.toggle_select_all_endpoints)
    ...
    def add_group(self):
        ...
        if ok and group_name and group_name not in self.group_widgets:
    ...
    def remove_files(self, file_list_widget):
        QtWidgets.QApplication.setOverrideCursor(QtCore.Qt.WaitCursor)
        file_list_widget.remove_selected_files()
        self.load_endpoints_from_files()
        QtWidgets.QApplication.restoreOverrideCursor()
    ...
    def load_endpoints_from_files(self):
        all_paths = []
        for file_list in self.group_widgets.values():
            all_paths.extend(file_list.get_full_paths())
        ...
        try:
            ...
            for path in all_paths:
                ...
                if common_endpoints is None:
                    common_endpoints = current_endpoints
                else:
                    common_endpoints.intersection_update(current_endpoints)
            
            if common_endpoints:
                for ep in sorted(list(common_endpoints)):
                    cb = QtWidgets.QCheckBox(ep); cb.setChecked(True)
                    self.endpoints_cb_layout.addWidget(cb)
    ...
    def toggle_select_all_endpoints(self):
        ...
        for i in range(self.endpoints_cb_layout.count()):
            if not self.endpoints_cb_layout.itemAt(i).widget().isChecked():
                all_selected = False; break
        for i in range(self.endpoints_cb_layout.count()):
    ...
    def start_analysis(self):
        ...
        for name, widget in self.group_widgets.items():
            ...
            if group_box.isChecked():
                ...
                if paths:
                    group_files[name] = paths
        
        if len(group_files) < 2:
            QtWidgets.QMessageBox.warning(self, "Input Error", "Please add files to at least two different active groups for comparison."); return
        if not self.output_dir_line_edit.text() or not os.path.isdir(self.output_dir_line_edit.text()):
            QtWidgets.QMessageBox.warning(self, "Input Error", "Please select a valid output directory."); return
        ...
        plot_params = {
            'central_tendency': self.central_tendency_combo.currentText(),
            'error_bar': self.error_bar_combo.currentText(), 'palette': self.palette_combo.currentText(),
            'width': self.plot_width_spinbox.value(), 'height': self.plot_height_spinbox.value(), 'dpi': self.plot_dpi_spinbox.value(),
            'title_size': self.title_size_spinbox.value(), 'axes_size': self.axes_size_spinbox.value(), 'tick_size': self.tick_size_spinbox.value(),
            'title_weight': self.title_weight_combo.currentText(), 'axes_weight': self.axes_weight_combo.currentText()
        }
        
        self.start_btn.setEnabled(False); self.report_text.clear(); self.plot_tabs.clear()

        self.worker = StatsProcessor(
            group_files=group_files,
            analysis_level=self.level_combo.currentText(),
            endpoints_to_analyze=selected_endpoints,
            normality_test=self.normality_test_combo.currentText(),
            alpha=self.alpha_spinbox.value(),
            force_parametric=self.force_parametric_cb.isChecked(),
            parametric_test=self.parametric_label.text(),
            nonparametric_test=self.nonparametric_label.text(),
            plot_params=plot_params,
            output_dir=self.output_dir_line_edit.text()
        )
        self.thread = QThread(); self.worker.moveToThread(self.thread)
        self.worker.finished.connect(self.on_analysis_finished); self.worker.plot_generated.connect(self.add_plot_tab)
        self.worker.log.connect(self.report_text.append); self.worker.error.connect(self.on_analysis_error); self.thread.started.connect(self.worker.run); self.thread.start()
